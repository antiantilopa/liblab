...
from engine.vmath_mini import Vector2d, from_bytes, to_bytes, print_bytes
...
class Obj_types:
    ...
    B = Obj_Type("B", 1, (250, 200, 40))
    C = Obj_Type("C", 1, (60, 100, 220))
    D = Obj_Type("D", 1, (250, 20, 20))
...
class Cell_Types:
    Amother = Cell_Type("A mother", 50,  (0, 120, 0),   (0, 40, 0, 0, 0), (4, 8, 0, 1))
    ...
    Cdigester = Cell_Type("C digester", 25, (0, 0, 60),  (35, 0, 0, 5, 0), initial_energy = 5)
    Ddigester = Cell_Type("D digester", 25, (60, 0, 0),  (45, 0, 0, 0, 5), initial_energy = 6)
...
def info_cell(screen:pygame.Surface, font: pygame.font.Font, typee:int, pos: tuple[float, float], mass: int, energy:int, owner:int, args:list = []):
    ...
    if len(args) == 0:
        ...
        if len(args[1]) != 0:
            ...
            for arg in args[1]:
                texts.append(font.render(f"  {Cell_Type.ALL[arg].name}", 0, Cell_Type.ALL[arg].color))
...
screen=pygame.display.set_mode((w, h), 16)

active_zone = pygame.Surface((w - (w)//4, h))
active_zone = pygame.transform.scale_by(active_zone, ratio)
...
while True:
            ...
            if event.button == 1:
                if event.pos[0] <= w - w//4:
                    ...
                    for obj in data[0]:
                        ...
                        objid = obj[3]
                        radius = Obj_Type.ALL[typee].radius_mass * mass
                        central_line = (Vector2d.from_tuple(event.pos)*ratio + camera - Vector2d.from_tuple(pos))
                    ...
                    if not hit:
                        info = -1
                        mouse_down = True
                        mouse_down_point = Vector2d.from_tuple(event.pos) * ratio + camera
                    else:
                        mouse_down = False
                else:
                    if info != -1:
                ...
                for cellid in selected:
                    send_info[0].append([cellid, (Vector2d.from_tuple(event.pos) * ratio + camera).operation(WORLD_SIZE, lambda a1, a2: min(max(a1, 0), a2))])
            ...
            if event.button == 1:
                if mouse_down:
                    ...
                    for cell in data[1]:
                        ...
                        if owner != pl:
                            continue
                        if Vector2d.from_tuple(pos).isInBox(mouse_down_point, Vector2d.from_tuple(event.pos)*ratio + camera):
                            selected.append(cellid)
                    if info == -1 and len(selected) == 1:
                        info = selected[0]
    ...
    if mouse_down:
        ...
        mx, nx = min(mx * ratio, nx), max(mx * ratio, nx)
        my, ny = min(my * ratio, ny), max(my * ratio, ny)
        pygame.draw.rect(active_zone, (200, 200, 200), (mx, my, nx-mx, ny-my), 5)
    screen.blit(pygame.transform.scale_by(active_zone, 1 / ratio), (0, 0))
    info_zone.blit(font.render(str(camera), 0, (250, 250, 120)), (0, 0))
